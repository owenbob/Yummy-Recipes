...
def register():
    ...
    if request.method == "POST":
        ...
        if name and email and password:
            ...
            flash("User already exists,Please go to Login") 
...
def edit_recipe(recipe_id):
    ...
    recipe_step = user.get_recipe(recipe_id)
    if not recipe_step:
    ...
    if request.method == "POST":
        ...
        if title and description:
            ...
            return render_template("edit_recipe.html", user=user, recipe_step=recipe_step)
...
def delete_recipe(recipe_id):
 ...
 #route enables the user to delete an existing recipe and all items items
    ...
    if request.method == "POST":
        if user.del_recipe(recipe_id):
            ...
            return redirect(url_for("delete_recipe", recipe_id=recipe_step.recipe_id))
        error = "Could not delete the Bucket"
    return render_template("delete_recipe.html", error=error, recipe_step=recipe_step, user=user)
...
def items(recipe_id):
    #route enables the users to create and view recipe items for specific recipes
    ...
    return render_template("items.html", user=user, recipe_step=recipe_step)
...
def delete_item(recipe_id, item_id):
    ...
    recipe_step = user.get_recipe(recipe_id)
    item = recipe_step.get_item(item_id)
    if not recipe_step and not item:
        flash("Item does not exist")
        return redirect(url_for("items", recipe_id=recipe_id))

    if request.method == "POST":
        if recipe_step.del_item(item_id):
            flash("Item has been deleted")
            return redirect(url_for("items", recipe_id=recipe_step.recipe_id))

        error = "Item was not deleted"
    return render_template("delete_items.html", error=error, user=user, recipe_step=recipe_step, item=item)
